...
export default function ControlPanel({
  ...
  const gistId = diagramId;
  const setGistId = setDiagramId;
  ...
  const save = async () => {
    if (typeof extensions.cloudSave === "function") {
      ...
      const isNew = routeDiagramId === "blank";
      const newId = isNew ? uuidv4() : routeDiagramId;
      const diagramData = {
        diagramId: newId,
        database,
        name: title,
  ...
  const saveAsCopy = async (newTitle) => {
    ...
    if (typeof extensions.cloudSave === "function") {
      ...
      try {
        await diagramsApi.create(diagramData);
        ...
        function: async () => {
          try {
            if (typeof extensions.cloudDelete === "function") {
              await extensions.cloudDelete(routeDiagramId);
            } else {
              await diagramsApi.remove(routeDiagramId);
              emit(TOPICS.DIAGRAMS_CHANGED);
            }
          ...
          >
            ...
            <div className="flex items-center gap-2 me-7">
              ...
              <span className="text-sm text-zinc-500 dark:text-zinc-400 hidden sm:inline">
                {user?.username}
                {user?.role === "admin" && (
                  <span className="ms-1 text-xs text-sky-600">(admin)</span>
                )}
              </span>
              {user?.role === "admin" && (
                <Button
                  type="tertiary"
                  className="!py-[18px] !rounded-md"
