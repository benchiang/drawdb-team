...
import { seedDefaultTemplatesIfEmpty } from "../hooks/useTemplates";
import {
  useLayout,
  useSettings,
  useTransform,
...
export default function WorkSpace({ forcedDiagramId } = {}) {
  ...
  const save = useCallback(async () => {
    ...
    if (isTemplate || (!loadedDiagramId && !isTemplate && !isDiagram)) {
      ...
      try {
        ...
        console.warn("save new diagram failed:", err);
        setSaveState(State.ERROR);
      }
    } else {
      try {
        await diagramsApi.update(loadedDiagramId, {
          database: database,
          name: title,
          lastModified: new Date(),
          tables: tables,
  ...
  const moveToCloud = useCallback(async () => {
    ...
    try {
      ...
      await diagramsApi.remove(loadedDiagramId);
      emit(TOPICS.DIAGRAMS_CHANGED);
  ...
  const load = useCallback(async () => {
    ...
    const fetchDiagram = async (id) => {
      let localDiagram = null;
      try {
        localDiagram = await diagramsApi.get(id);
      } catch (err) {
        if (err?.response?.status !== 404) {
          console.warn("fetch diagram failed:", err);
        }
        localDiagram = null;
      }
    ...
    const applyDiagramState = (diagram) => {
      ...
      setDiagramId(diagram.diagramId);
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden theme">
      <IdContext.Provider value={{ diagramId, setDiagramId, version, setVersion }}>
        <ControlPanel
          title={title}
          setTitle={setTitle}
          lastSaved={lastSaved}
